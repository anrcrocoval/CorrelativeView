 ...
 * @author Stephane
 */
public class TestVisu extends IcyCanvas2D implements ROITaskListener
{
    /**
	 * 
	 */
    ...
    /**
     * Possible rounded zoom factor : 0.01 to 100
     */
     ...
     */
    protected class TestVisuImageOverlay extends IcyCanvasImageOverlay
    {
        @Override
        public void paint(Graphics2D g, Sequence sequence, IcyCanvas canvas)
        {
            ...
            final List<plugins.perrine.correlativeview.TestVisu.CanvasView.ImageCache.ImageCacheTile> tiles = canvasView.imageCache.getImageAsTiles();

            // draw image
            for (plugins.perrine.correlativeview.TestVisu.CanvasView.ImageCache.ImageCacheTile tile : tiles)
            ...
            implements ActionListener, MouseWheelListener, MouseListener, MouseMotionListener
    {
        ...
        public class ImageCache implements Runnable
        {
            ...
            public ImageCache()
            {
                ...
                processor = new SingleProcessor(true, "TestVisu renderer");
            ...
            public void run()
            {
                ...
                // get original image
                final IcyBufferedImage icyImage = TestVisu.this.getImage(getPositionT(), getPositionZ(),
                        getPositionC());
        ...
        void drawLayer(Graphics2D g, Sequence seq, Layer layer)
        {
            if (layer.isVisible())
            {
                ...
                layer.getOverlay().paint(g, seq, TestVisu.this);
            }
        }
        ...
        protected void paintComponent(Graphics g)
        {
            ...
            // background and layers
            {
                ...
                AffineTransform transformtocanvas = getTransform();
                // apply filtering
                ...
                g2.dispose();
                /** adding for correlated image**/
                final Graphics2D gcor = (Graphics2D) g.create();

                // background
                if (isBackgroundColorEnabled())
                {
                    gcor.setBackground(getBackgroundColor());
                ...
                if (CanvasPreferences.getFiltering() && ((getScaleX() < 4d) && (getScaleY() < 4d)))
                    ...
                    gcor.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
                            RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR);
                ...
                Ts.setToScale(scales/scalet, scales/scalet);
                AffineTransform Tt=new AffineTransform();
               //double ttrx=(w/2)+(-20.72149107863949/0.11)-(1392)*0.11;
               //double ttry=(h/2)+(111.79450167515574/0.11)-(1040)*0.11;
              //double ttrx=968;
              //double ttry=373;
              double ttrx=matrix.get(0, 3)*(1/(scales));
              double ttry=matrix.get(1, 3)*(1/(scales));
               ...
               //myTransform.concatenate(Tt);
				gcor.transform(myTransform);
				
                // draw image and layers
                
               
               gcor.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1-getImageLayer().getOpacity()));
               gcor.drawImage(correlatedimage,0,0,null);
               //gcor.dispose();
         ...
         * @return Rectangle the area selection
         */
     ...
     * @author Stephane
     */
    static class TestVisuSmoothMover extends MultiSmoothMover
    {
        public TestVisuSmoothMover(int size, SmoothMoveType type)
        {
            super(size, type);
        }
    ...
    public TestVisu(Viewer viewer, BufferedImage correlatedimagebuf, double[][] m, double ss, double st)
    {
        ...
        preferences = CanvasPreferences.getPreferences().node(PREF_TestVisu_ID);

        // init transform (5 values, log transition type)
        smoothTransform = new TestVisuSmoothMover(5, SmoothMoveType.LOG);
        ...
        getImageLayer().setOpacity((float) 0.8);
    ...
    public void shutDown()
    {
        ...
        // shutdown mover object (else internal timer keep a reference to TestVisu)
    ...
    {
        return (TestVisuSettingPanel) panel;
    }
    ...
    private void buildSettingGUI()
    {
        ...
        panel = new TestVisuSettingPanel(this);
        // add the map to it
    ...
    /**
     * Return the {@link CanvasView} component of TestVisu.
     */
    ...
    /**
     * Return the {@link CanvasMap} component of TestVisu.
     */
    ...
    {
        final IcyBufferedImage img = TestVisu.this.getImage(t, z, c);
